 ...
 * <p>
 * DefaultTreeCellRende rer 类继承了JLabel，
 * 实现 getTreeCellRendererComponent()方法时返回 this ，
 * 即返回一个特殊的 JLabel 对象 。
 * 如果需要根据节点内容来改变节点的外观，
 * 则可以再次扩展DefaultTreeCellRenderer 类，并
 * 再次重写它提供的 getTreeCellRendererComponent()方法。
 * <p>
 * 自定义类继承DefaultTreeCellRenderer,重写getTreeCellRendererComponent()方法
 *
 * @author : zxq
 * @create : 2022/8/12 0:22
 */
public class ExtendsDefaultCellTreeRenderer {
    JFrame jf = new JFrame("根据结点类型定义图标");
    JTree tree;
    //初始化5个图标
    ImageIcon rootIcon = new ImageIcon(this.getClass().getClassLoader().getResource("img/root.gif"));
    ImageIcon databaseIcon = new ImageIcon(this.getClass().getClassLoader().getResource("img/database.gif"));
    ImageIcon tableIcon = new ImageIcon(this.getClass().getClassLoader().getResource("img/table.gif"));
    ImageIcon columnIcon = new ImageIcon(this.getClass().getClassLoader().getResource("img/column.gif"));
    ImageIcon indexIcon = new ImageIcon(this.getClass().getClassLoader().getResource("img/index.gif"));
